		...
		<v-card flat class="invoice-section-card pos-themed-card pos-list-card">
			<div class="pos-list-header">
				<div class="pos-list-header__main">
					...
					<h3 class="pos-list-header__title">{{ __("Weekly Wastage") }}</h3>
					<p class="pos-list-header__subtitle">
						{{ __("Wastage over 1% of the period's work, charged to workers by points") }}
					</p>
				</div>
			...
			<div class="pos-list-stats">
				...
				<div class="pos-list-stat pos-list-stat--warning">
					<span class="pos-list-stat__label">{{ __("Actual Wastage (KG)") }}</span>
					<strong class="pos-list-stat__value">{{ formatFloat(totalWastageKg) }}</strong>
				</div>
				<div class="pos-list-stat pos-list-stat--success">
					<span class="pos-list-stat__label">{{ __("Total Penalty") }}</span>
					<strong class="pos-list-stat__value">{{ formatCurrency(totalPenalty) }}</strong>
				</div>
			</div>
			...
			<div v-if="records.length" class="pos-list-table-wrap">
				...
				>
					...
					<template #item.period="{ item }">
						<span class="pos-list-cell-muted">
							{{ formatDisplayDate(item.start_date) }} → {{ formatDisplayDate(item.end_date) }}
						</span>
					</template>
					<template #item.total_weekly_work_kg="{ item }">{{ formatFloat(item.total_weekly_work_kg) }}</template>
					<template #item.allowed_wastage_kg="{ item }">{{ formatFloat(item.allowed_wastage_kg) }}</template>
					<template #item.actual_wastage_kg="{ item }">{{ formatFloat(item.actual_wastage_kg) }}</template>
					<template #item.excess_wastage_kg="{ item }">{{ formatFloat(item.excess_wastage_kg) }}</template>
					<template #item.penalty_amount="{ item }">{{ formatCurrency(item.penalty_amount) }}</template>
			...
			<div v-else-if="!listLoading" class="pos-list-empty">
				<v-icon size="48" color="primary" class="pos-list-empty__icon">mdi-delete-variant</v-icon>
				<h4 class="pos-list-empty__title">{{ __("No wastage records found") }}</h4>
				<p class="pos-list-empty__subtitle">
					{{ hasActiveFilters
						? __("Try different filters or clear them.")
						: __("Record a period's molding wastage to see it listed here.") }}
				</p>
			</div>
...
export default {
	...
	setup() {
		...
		const listHeaders = [
			{ title: __('Wastage'), key: 'name', sortable: true },
			{ title: __('Period'), key: 'period', sortable: false },
			{ title: __('Work (KG)'), key: 'total_weekly_work_kg', sortable: true, align: 'end' },
			{ title: __('Allowed (KG)'), key: 'allowed_wastage_kg', sortable: true, align: 'end' },
			{ title: __('Actual (KG)'), key: 'actual_wastage_kg', sortable: true, align: 'end' },
			{ title: __('Excess (KG)'), key: 'excess_wastage_kg', sortable: true, align: 'end' },
			{ title: __('Penalty'), key: 'penalty_amount', sortable: true, align: 'end' },
		...
		const totalWastageKg = computed(() =>
			activeRecords.value.reduce((sum, row) => sum + (Number(row.actual_wastage_kg) || 0), 0),
		);
		const totalPenalty = computed(() =>
			activeRecords.value.reduce((sum, row) => sum + (Number(row.penalty_amount) || 0), 0),
		);
		...
		const loadRecords = async () => {
			...
			try {
				const { message } = await frappe.call({
					method: `${API}.get_weekly_wastage_list`,
					args: {
				...
				console.error('Failed to load molding weekly wastage', e);
				records.value = [];
			} finally {
		...
		const goToNew = () => router.push('/molding-wastage/new');
		const openDetail = (item) => router.push(`/molding-wastage/${item.name}`);

		const actionLoadingName = ref(null);
		const confirmDialog = ref(false);
		const confirmItem = ref(null);
		const confirmMessage = computed(() =>
			__('This will cancel {0} and its Additional Salary penalty deductions. Continue?', [
		...
		const runCancel = async () => {
			...
			try {
				await frappe.call({ method: `${API}.cancel_weekly_wastage`, args: { name: item.name } });
